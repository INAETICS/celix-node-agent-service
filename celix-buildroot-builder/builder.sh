...
add_user_in_container() {
    BUILDER_USER=celix-user
    BUILDER_GROUP=celix-group
...
command:help() {
    if [[ $# != 0 ]]; then
        ...
        cat >&2 <<ENDHELP
usage: docker run ${THIS_IMAGE} <command> <args> or
...
Built-in commands for "docker run ${THIS_IMAGE}":
...
command:make_celix_agent() {
    mkdir -p /tmp/celix-image
    container=$(docker create ${THIS_IMAGE})
    docker cp ${container}:/usr/buildroot/output/images/rootfs.tar /tmp/celix-image/
    docker rm ${container}
    cat /tmp/celix-image/rootfs.tar | docker import - ${FINAL_IMAGE}
}

#command:build() {
#    docker run -v /tmp/cagent:/build $THIS_IMAGE /bin/bash -c "cp /usr/celix-image/rootfs.tar /build/."
...
command:make_bundles() {
    ...
    echo "set(CMAKE_SYSROOT \"/usr/buildroot/output/host/usr/x86_64-buildroot-linux-gnu/sysroot\")" >> ./toolchain.cmake
    echo "set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} \"/usr/buildroot/output/host/usr/x86_64-buildroot-linux-gnu/sysroot/usr/share/celix/cmake/modules\")" >> ./toolchain.cmake
    echo "include(/usr/buildroot/output/host/usr/share/buildroot/toolchainfile.cmake)" >> ./toolchain.cmake
    # Start docker run inaetics/cagent_builder with command to run cmake
    cd ..
    docker run --rm --privileged -v $PWD:/build ${USER_IDS} $THIS_IMAGE build_bundles $@
...
#    docker run -v /tmp/celix_bundles:/build ${USER_IDS} ${THIS_IMAGE} /bin/bash -c "cp /usr/buildroot/output/target/usr/share/celix/bundles/*.zip /build/."
    docker run --rm --privileged -v /tmp/celix_bundles:/build ${USER_IDS} ${THIS_IMAGE} build_agent_bundles 
...
command:build_bundles() {
    add_user_in_container
    cd /build/build
    chpst -u :$BUILDER_UID:$BUILDER_GID cmake -DCMAKE_TOOLCHAIN_FILE=./toolchain.cmake -DCELIX_DIR=/usr/buildroot/output/target/usr $@ ..
    chpst -u :$BUILDER_UID:$BUILDER_GID make
}

command:build_script() {
    cat /usr/buildroot/builder.sh
    exit 0
}

command:build_agent_bundles() {
    add_user_in_container
    exec chpst -u :$BUILDER_UID:$BUILDER_GID cp /usr/buildroot/output/target/usr/share/celix/bundles/*.zip /build/.

}



THIS_IMAGE="inaetics/celix-buildroot-builder"
FINAL_IMAGE="inaetics/celix"
...
case $1 in
    ...
    *)
      if has command $1; then
          ...
          docker run --rm --privileged -i -t -v $PWD:/build --entrypoint=$1 ${THIS_IMAGE} ${@:2}
      fi
      ;;
esac
